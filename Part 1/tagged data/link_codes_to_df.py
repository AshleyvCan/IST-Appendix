...
# Open txt files (exported files from Nvivo)
def open_codes(file_name):
    with codecs.open(file_name,  'r', encoding="utf-16") as f:
        lines = f.read()
    return lines

# Remove specific punctuation/characters from string
def replace_column_char(df,char):
    for c in char:
        df = df.str.replace(c, '')
    return df

# Print requirement if it is a duplicate
...
def link_req_to_df(project_names, categories_per_item, codes_2):
    ...
    for project_name in project_names:
        df = pd.read_excel('original_samples/'+ project_name + '.xlsx')
...
def main(dir):
    ...
    # Join the tags of the summary and description together.
    #df_all.to_excel('temp/sum_and_desc_count.xlsx')
