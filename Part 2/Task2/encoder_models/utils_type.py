...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, df, tokenizer, labels):
        self.labels = [labels[label] for label in df['label_type']]
...
def evaluate(model, test_data, tokenizer, labels, name_model):
    unique_classes = set(test_data['label_type'])
    ...
    export_performance(name_model, unique_classes, total_acc_test, test_data, TP, FP, FN)
...
def main(datapath, tokenizer, labels, name_model):
    ...
    for val_project in all_val_projects: 
        ...
        df_train = df.loc[~df['project'].isin([val_project, test_project]), ['text', 'label_type']]
        df_val = df.loc[df['project'] == val_project, ['text', 'label_type']]

        # Run model
        EPOCHS = 10
        LR = 1e-5
        print(name_model+val_project)            
        epoch_performance = train(model_base, df_train, df_val, LR, EPOCHS,tokenizer, labels,name_model + '_'+ val_project, name_model)
        epoch_performance.to_excel(name_model+'/'+name_model+ '_'+ val_project + '_epochs.xlsx')
